...
export function Navigation({ onScrollTo, isHomePage }: Props) {
  ...
  return <header className={`nav-header ${scrolled?'nav-scrolled':''}`}>
    <Link to="/" className="nav-brand"><img className="nav-logo-image" src="/ns-auto-luxe-logo.png" alt="North Splash Auto Luxe"/><div><strong>NORTH SPLASH</strong><small>AUTO LUXE</small></div></Link>
    ...
    <div className="nav-right"><a className="nav-cta nav-portal-link" href={`${OS_URL}/login`}><LogIn size={15}/><span>Portal</span></a><button className="nav-book" onClick={()=>go('booking')}>Book Now</button><button className="menu-toggle" onClick={()=>setMenuOpen(!menuOpen)}>{menuOpen?<X size={22}/>:<Menu size={22}/>}</button></div>
  </header>;
}
