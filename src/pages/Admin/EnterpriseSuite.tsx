...
export default function EnterpriseSuite({ section, employees, setEmployees, appointments, setAppointments }: Props) {
  ...
  const [territoryForm, setTerritoryForm] = useState({ name: '', assigned_employee_id: '', center_lat: '', center_lng: '', radius_meters: 1200, notes: '' });
  ...
  const addTerritory = async (e: React.FormEvent) => {
    e.preventDefault();
    const { data, error } = await supabase.from('lead_territories').insert({ ...territoryForm, assigned_employee_id: territoryForm.assigned_employee_id || null, center_lat: territoryForm.center_lat ? Number(territoryForm.center_lat) : null, center_lng: territoryForm.center_lng ? Number(territoryForm.center_lng) : null }).select().single();
  ...
  if (section === 'territories') return <div className="tab-content"><Header title="Territories" subtitle="Assign neighborhoods/areas to D2D reps. Center + radius creates a visible map territory." />
    <div style={{display:'grid',gridTemplateColumns:'minmax(320px,.5fr) minmax(0,1.5fr)',gap:18}}><form style={card} onSubmit={addTerritory}><h3><MapPin size={18}/> New Territory</h3><input required placeholder="Territory name" value={territoryForm.name} onChange={e=>setTerritoryForm(p=>({...p,name:e.target.value}))}/><select value={territoryForm.assigned_employee_id} onChange={e=>setTerritoryForm(p=>({...p,assigned_employee_id:e.target.value}))}><option value="">Unassigned</option>{employees.filter(e=>e.role==='d2d_agent').map(e=><option value={e.id} key={e.id}>{e.name}</option>)}</select><input placeholder="Center latitude" value={territoryForm.center_lat} onChange={e=>setTerritoryForm(p=>({...p,center_lat:e.target.value}))}/><input placeholder="Center longitude" value={territoryForm.center_lng} onChange={e=>setTerritoryForm(p=>({...p,center_lng:e.target.value}))}/><input type="number" min="100" step="100" value={territoryForm.radius_meters} onChange={e=>setTerritoryForm(p=>({...p,radius_meters:Number(e.target.value)}))}/><textarea placeholder="Notes" value={territoryForm.notes} onChange={e=>setTerritoryForm(p=>({...p,notes:e.target.value}))}/><button className="btn-primary btn-full">Create Territory</button></form><LeadMap leads={leads} territories={territories} onMapPoint={(lat,lng)=>setTerritoryForm(p=>({...p,center_lat:lat.toFixed(6),center_lng:lng.toFixed(6)}))}/></div>
    <div style={{...grid,marginTop:18}}>{territories.map(t=><div style={card} key={t.id}><strong>{t.name}</strong><p>{empName(t.assigned_employee_id)}</p><span>{t.radius_meters.toLocaleString()}m radius</span></div>)}</div></div>;
...
function Permissions({ profiles, employees, setEmployees, onSave }: { profiles: Profile[]; employees: Employee[]; setEmployees: React.Dispatch<React.SetStateAction<Employee[]>>; onSave: (person: Profile, role: PortalRole, permissions: Record<string, boolean>) => Promise<void> }) {
  ...
  return <div className="tab-content"><Header title="Portal Permissions" subtitle="Choose which portal each account enters and exactly what that person is allowed to access." />
