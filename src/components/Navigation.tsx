...
export function Navigation({ onScrollTo, isHomePage }: Props) {
  ...
  return <header className={`nav-header ${scrolled?'nav-scrolled':''}`}>
    <Link to="/" className="nav-brand"><div className="brand-mark">NS</div><div><strong>NORTH SPLASH</strong><small>AUTO LUXE</small></div></Link>
    <nav className={`nav-links ${menuOpen?'nav-open':''}`}><button onClick={()=>go('services')}>Services</button><button onClick={()=>go('packages')}>Packages</button><button onClick={()=>go('protection')}>Protection</button><button onClick={()=>go('membership')}>Membership</button><button onClick={()=>go('contact')}>Contact</button></nav>
